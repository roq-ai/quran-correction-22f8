...
type ColumnType = ColumnDef<UserProgressInterface, unknown>;

interface UserProgressListPageProps {
...
export function UserProgressListPage(props: UserProgressListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<UserProgressInterface>>(
    () => `/user-progresses?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: UserProgressInterface) => {
    if (hasAccess('user_progress', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/user-progresses/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/user-progresses/view/${record.id}`} passHref legacyBehavior>
                <Button
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="user_progress" />
        </Flex>
