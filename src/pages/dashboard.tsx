...
import { getUserProgresses } from 'apiSdk/user-progresses';
import { UserProgressInterface } from 'interfaces/user-progress';
import { getCorrectionHistories } from 'apiSdk/correction-histories';
import { CorrectionHistoryInterface } from 'interfaces/correction-history';
import { getReadingSessions } from 'apiSdk/reading-sessions';
import { ReadingSessionsInterface } from 'interfaces/reading-sessions';

import { ReadingSessionsListPage as ShowCaseList } from 'pages/reading-sessions';

function DashboardPage() {
  const { session } = useSession();

  const userProgressStat = useSWR<PaginatedInterface<UserProgressInterface>>(
    () => `/user-progresses`,
    () => getUserProgresses({ limit: 1, offset: 0 }),
  );

  const correctionHistoryStat = useSWR<PaginatedInterface<CorrectionHistoryInterface>>(
    () => `/correction-histories`,
    () => getCorrectionHistories({ limit: 1, offset: 0 }),
  );

  const readingSessionsStat = useSWR<PaginatedInterface<ReadingSessionsInterface>>(
    () => `/reading-sessions`,
    () => getReadingSessions({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'User Progresses',
      result: userProgressStat?.data?.totalCount,
      isLoading: userProgressStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Correction Histories',
      result: correctionHistoryStat?.data?.totalCount,
      isLoading: correctionHistoryStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Reading Sessions',
      result: readingSessionsStat?.data?.totalCount,
      isLoading: readingSessionsStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`reading-sessions`} passHref legacyBehavior>
                  <Button
